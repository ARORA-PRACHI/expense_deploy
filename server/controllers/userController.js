...
const changePasswordController = async (req, res) => {
    try {
        // Extract email, currentPassword, and newPassword from the request body
        const { email, currentPassword, newPassword } = req.body;

        // Validate input fields
        if (!email || !currentPassword || !newPassword) {
            return res.status(400).send(error(400, "Email, current password, and new password are required!"));
        }
        ...
        if (!isMatch) {
            return res.status(400).send(error(400, "Current password is incorrect!"));
        }
        ...
        return res.status(200).send(success(200, "Password changed successfully!"));
    } catch (err) {
        return res.status(500).send(error(500, err.message));
    }
};
